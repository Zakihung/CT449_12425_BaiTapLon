const express = require('express');
const cors = require('cors');
const ApiError = require("./app/api-error");

const app = express();
app.use(cors());
app.use(express.json());

app.get("/", (req, res) => {
    res.json({ message: "Chào mừng" });
});

const NhaXuatBanRouter = require("./app/routes/NhaXuatBan.route");
const TaiKhoanRouter = require("./app/routes/TaiKhoan.route");
const NhanVienRouter = require("./app/routes/NhanVien.route");
const SachRouter = require("./app/routes/Sach.route");
const TheoDoiMuonSachRouter = require("./app/routes/TheoDoiMuonSach.route");
const DocGiaRouter = require("./app/routes/DocGia.route");
const LoaiSachRouter = require("./app/routes/LoaiSach.route");
const TacGiaRouter = require("./app/routes/TacGia.route")

app.use("/api/nxb", NhaXuatBanRouter);
app.use("/api/taikhoan", TaiKhoanRouter);
app.use("/api/nhanvien", NhanVienRouter);
app.use("/api/sach", SachRouter);
app.use("/api/docgia", DocGiaRouter);
app.use("/api/loaisach", LoaiSachRouter);
app.use("/api/tdms", TheoDoiMuonSachRouter);
app.use("/api/tacgia", TacGiaRouter);

const CounterDG = require("./app/models/CounterDG.model");

const initCounter = async () => {
    const counter = await CounterDG.findById('nxbid');
    if (!counter) { 
        const newCounter = new CounterDG({ _id: 'nxbid' }); 
        await newCounter.save(); 
    }
}
initCounter().catch(err => console.error(err));

// handle 404 response
app.use((req, res, next) => {
    // Code ở đây sẽ chạy khi không có route được định nghĩa nào
    // khớp với yêu cầu. Gọi next() để chuyển sang middleware xử lý lỗi
    return next(new ApiError(404, "Resource not found"));
});

// define error-handling middleware last, after other app.use() and routes calls
app.use((err, req, res, next) => {
    // Middleware xử lý lỗi tập trung.
    // Trong các đoạn code xử lý ở các route, gọi next(error)
    // sẽ chuyển về middleware xử lý lỗi này
    return res.status(err.statusCode || 500).json({
        message: err.message || "Internal Server Error",
    });
});


module.exports = app;